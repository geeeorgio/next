      ...
      <div className="flex flex-col md:flex-row pl-3 pr-3 md:pl-4 md:pr-4 lg:pl-6 lg:pr-6">
        <ul className="relative w-full flex flex-col md:flex-row gap-3 md:gap-4 lg:gap-5 xl:gap-6">
          {servicesList.map(({ label_ua, desc, icon: Icon }) => (
            <li
              key={label_ua}
              className="relative overflow-hidden flex flex-col flex-1 rounded-xl border border-gold-accent/20 bg-linear-to-b from-primary-section/55 via-primary-dark/65 to-primary-deep/90 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] gap-4 md:gap-5 lg:gap-6 xl:gap-7 p-5 lg:p-7 transition-all duration-500 hover:border-gold-accent/30 hover:shadow-[0_25px_60px_-25px_rgba(214,176,106,0.28)]"
            >
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="flex flex-col gap-3">
                  <h3 className="text-foreground/95 text-base lg:text-xl font-semibold tracking-wide leading-snug">
                    {label_ua}
                  </h3>

                  <p className="text-sm md:text-base leading-relaxed text-foreground-muted/90">
                    {desc}
                  </p>
                </div>
              </div>
